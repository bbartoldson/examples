...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        ...
        in/.style={mol, fill=teal!8, draw=teal!45},
        step/.style={mol, fill=teal!24, draw=teal!60},
        out/.style={mol, fill=teal!45, draw=teal!75, font=\footnotesize\bfseries},
        ...
        \node[in] (m1) {Mol-1};
        \node[in, below=0.35cm of m1] (m2) {Mol-2};

        \node[in, below=1.1cm of m2] (m6) {Mol-6};
        \node[in, below=0.35cm of m6] (m7) {Mol-7};

        % First level products
        \node[step, right=1.6cm of m1.east, yshift=-0.175cm] (m3) {Mol-3};
        \node[step, right=1.6cm of m6.east, yshift=-0.175cm] (m8) {Mol-8};

        % Second input
        \node[in, right=1.6cm of m3.east] (m4) {Mol-4};

        % Second level product
        \node[step, right=1.6cm of m4.east, yshift=-0.8cm] (m5) {Mol-5};

        % Final output
        \node[out, right=2.0cm of m5.east, yshift=-0.3cm] (final) {Output};
